...
public class PardonCommand implements DiscordCommand {
	...
	public boolean execute(Message message, String[] command) {
		...
		if(command.length == 1) {
			...
			if(command[0].equalsIgnoreCase("pardonplayer")) {
				...
				Message cache = message.getChannel().sendMessage(
						new EmbedBuilder()
							...
							.setDescription(
									message.getAuthor().getAsMention() + " El comando `pardonplayer` debe tener especificado como parámetros " + 
									"el nombre del jugador. \n" +
									"**Ejemplo:** `::pardonplayer Notch`")
		...
		if(command[0].equalsIgnoreCase("pardonplayer")) {
			...
			Message cache = DHandlers.BOT.cmdChannel.sendMessage(
					new EmbedBuilder()
						...
						.setDescription(
								...
								"**Comando: ** Perdonar baneo `pardonplayer` \n" +
								"**Jugador: ** " + command[1])
			...
			Packets.STREAMER.streamPacketAndRespose(
					new RequestActionExecutorPacket()
						.setRequestAction("pardonrequest")
