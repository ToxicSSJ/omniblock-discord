...
public class NetworkIDCommand implements DiscordCommand {
	...
	public boolean execute(Message message, String[] command) {
		...
		if(command.length == 1) {
			...
			if(command[0].equalsIgnoreCase("getplayerid")) {
				...
				Message cache = message.getChannel().sendMessage(
						new EmbedBuilder()
							...
							.setDescription(
									message.getAuthor().getAsMention() + " El comando `getplayerid` debe tener especificado como parámetro " + 
									"el nombre del jugador. \n" +
									"**Ejemplo:** `::getplayerid Notch`")
		...
		if(command[0].equalsIgnoreCase("getplayerid")) {
			...
			Message cache = DHandlers.BOT.cmdChannel.sendMessage(
					new EmbedBuilder()
						...
						.setDescription(
								...
								"**Comando: ** Recibir la NetworkID de un Jugador `getplayerid` \n" +
								"**Jugador: ** " + command[1])
			...
			Packets.STREAMER.streamPacketAndRespose(
					new RequestActionExecutorPacket()
						.setRequestAction("networkidrequest")
