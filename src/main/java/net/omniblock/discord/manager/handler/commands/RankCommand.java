...
public class RankCommand implements DiscordCommand {
	...
	public boolean execute(Message message, String[] command) {
		...
		if(command.length >= 1 && command.length <= 2) {
			...
			if(command[0].equalsIgnoreCase("rankplayer")) {
				...
				Message cache = message.getChannel().sendMessage(
						new EmbedBuilder()
							...
							.setDescription(
									message.getAuthor().getAsMention() + " El comando `rankplayer` debe tener especificado como parámetros " + 
									"el nombre del jugador y el nombre del rango que le darás. \n" +
									"**Ejemplo:** `::rankplayer Notch Moderador`")
			...
			Message cache = DHandlers.BOT.cmdChannel.sendMessage(
					new EmbedBuilder()
						...
						.setDescription(
								...
								"**Comando: ** Elegir el Rango `rankplayer` \n" +
								"**Jugador: ** " + command[1] + " \n" +
								"**Rango: ** " + command[2])
