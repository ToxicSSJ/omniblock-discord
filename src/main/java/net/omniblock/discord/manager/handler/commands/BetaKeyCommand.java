...
public class BetaKeyCommand implements DiscordCommand {
	...
	public boolean execute(Message message, String[] command) {
		...
		if(command.length == 1) {
			...
			if(command[0].equalsIgnoreCase("betakey")) {
				...
				Message cache = message.getChannel().sendMessage(
						new EmbedBuilder()
							...
							.setDescription(
									message.getAuthor().getAsMention() + " El comando `betakey` debe tener especificado como parámetro " + 
									"el nombre del jugador. \n" +
									"**Ejemplo:** `::betakey Notch`")
		...
		if(command[0].equalsIgnoreCase("betakey")) {
			...
			Message cache = DHandlers.BOT.cmdChannel.sendMessage(
					new EmbedBuilder()
						...
						.setDescription(
								...
								"**Comando: ** Generar una Beta Key `betakey` \n" +
								"**Jugador: ** " + command[1] + " \n")
			...
			Packets.STREAMER.streamPacketAndRespose(
					new RequestActionExecutorPacket()
						.setRequestAction("getbetakeyrequest")
