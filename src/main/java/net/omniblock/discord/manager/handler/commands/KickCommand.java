...
public class KickCommand implements DiscordCommand {
	...
	public boolean execute(Message message, String[] command) {
		...
		if(command.length >= 1 && command.length <= 2) {
			...
			if(command[0].equalsIgnoreCase("kickplayer")) {
				...
				Message cache = message.getChannel().sendMessage(
						new EmbedBuilder()
							...
							.setDescription(
									message.getAuthor().getAsMention() + " El comando `kickplayer` debe tener especificado como parámetros " + 
									"el nombre del jugador y el mensaje que le enviarás. \n" +
									"**Ejemplo:** `::kickplayer Notch ¡No utilices hacks!`")
			...
			Message cache = DHandlers.BOT.cmdChannel.sendMessage(
					new EmbedBuilder()
						...
						.setDescription(
								...
								"**Comando: ** Expulsar a un Jugador `kickplayer` \n" +
								"**Jugador: **" + command[1] + " \n" +
								"**Razón: ** " + reason)
