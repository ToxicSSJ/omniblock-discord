...
public class ForceBoosterCommand implements DiscordCommand {
	...
	public boolean execute(Message message, String[] command) {
		...
		if(command.length == 1) {
			...
			if(command[0].equalsIgnoreCase("forceboosterplayer")) {
				...
				Message cache = message.getChannel().sendMessage(
						new EmbedBuilder()
							...
							.setDescription(
									message.getAuthor().getAsMention() + " El comando `forceboosterplayer` debe tener especificado como parámetros " + 
									"el nombre del jugador. \n" +
									"**Ejemplo:** `::forceboosterplayer Notch`")
		...
		if(command[0].equalsIgnoreCase("forceboosterplayer")) {
			...
			Message cache = DHandlers.BOT.cmdChannel.sendMessage(
					new EmbedBuilder()
						...
						.setDescription(
								...
								"**Comando: ** Forzar un Network Booster (3H) `forceboosterplayer` \n" +
								"**Jugador: ** " + command[1])
			...
			Packets.STREAMER.streamPacketAndRespose(
					new RequestActionExecutorPacket()
						.setRequestAction("forceboosterplayerrequest")
					...
					new PacketResponder<ResposeActionExecutorPacket>() {
						...
						public void readRespose(PacketSocketData<ResposeActionExecutorPacket> packetsocketdata) {
							...
							MessageEmbed embed = new EmbedBuilder()
									...
									.setDescription(
											"\n \n**Ejecutor: ** " + message.getAuthor().getAsMention() + " \n" +
											"**Comando: ** Forzar un Network Booster (6H) `forceboosterplayer` \n" +
											"**Jugador: ** " + command[1] + " \n" +
