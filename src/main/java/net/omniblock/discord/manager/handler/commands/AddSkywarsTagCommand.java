...
public class AddSkywarsTagCommand implements DiscordCommand {
	...
	public boolean execute(Message message, String[] command) {
		...
		if(command.length >= 1 && command.length <= 2) {
			...
			if(command[0].equalsIgnoreCase("addskywarstag")) {
				...
				Message cache = message.getChannel().sendMessage(
						new EmbedBuilder()
							...
							.setDescription(
									message.getAuthor().getAsMention() + " El comando `addskywarstag` debe tener especificado como parámetros " + 
									"el nombre del jugador y el tag que le darás. \n" + 
									"**Ejemplo:** `::addskywarstag Notch J5`")
		...
		if(command[0].equalsIgnoreCase("addskywarstag")) {
			...
			Message cache = DHandlers.BOT.cmdChannel.sendMessage(
					new EmbedBuilder()
						...
						.setDescription(
								...
								"**Comando: ** Añadir un tag de Skywars a un Jugador `addskywarstag` \n" +
								"**Jugador: **" + command[1] + " \n" +
								"**Tag: ** " + command[2])
			...
			Packets.STREAMER.streamPacketAndRespose(
					new RequestActionExecutorPacket()
						.setRequestAction("addskywarstagrequest")
